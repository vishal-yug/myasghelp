...
function porto_preprocess_page(&$vars, $hook) {
  ...
  if (request_path() == 'one-page') {
	...
	drupal_add_js(drupal_get_path('theme', 'porto') . '/vendor/elevatezoom/jquery.elevatezoom.js',array('type' => 'file','scope' => 'footer'));
	drupal_add_js(drupal_get_path('theme', 'porto') . '/js/demos/demo-shop-6.js',array('type' => 'file','scope' => 'footer'));
...
function porto_menu_link__menu_mobile(array $variables) {
    $output = '';
    unset($variables['element']['#attributes']['class']);
    $element = $variables['element'];
    static $item_id = 0;
    ...
    $depth = $element['#original_link']['depth'];
    if ( ($element['#below']) && ($depth == "1") ) {
        $output.= '<span class="mmenu-toggle"></span>';
        $element['#attributes']['class'][] = 'dropdown '.$element['#original_link']['mlid'].'';
        $element['#localized_options']['attributes']['class'][] = 'dropdown-toggle disabled';
    }
...
function porto_menu_tree__header_menu($variables){
  return '<nav><ul class="nav nav-pills" id="mainNav">' . $variables['tree'] . '</ul></nav>';
...
function porto_menu_tree__menu_mobile($variables){
    return '<ul class="mobile-side-menu">' . $variables['tree'] . '</ul>';
}


function porto_menu_tree__menu_mobile_below($variables){
...
function porto_form_alter(&$form, &$form_state, $form_id) {
  ...
  }elseif(stristr($form_id,'comment_node')){
...
function porto_block_view_alter(&$data, $block) {
  ...
  if ( ($block->region == 'menu_mobile') && !isset($data['content']['#type']) ) {
        $data['content']['#theme_wrappers'] = array('menu_tree__menu_mobile');

        foreach($data['content'] as &$key):

            if (isset($key['#theme'])) {
                $key['#theme'] = 'menu_link__menu_mobile';
            }
            if (isset($key['#below']['#theme_wrappers'])) {
                $key['#below']['#theme_wrappers'] = array('menu_tree__menu_mobile_below');
            }
            if (isset($key['#below'])) {
                foreach($key['#below'] as &$key2):

                    if (isset($key2['#theme'])) {
                        $key2['#theme'] = 'menu_link__menu_mobile';
                    }
                    if (isset($key2['#below']['#theme_wrappers'])) {
                        $key2['#below']['#theme_wrappers'] = array('menu_tree__menu_mobile_below');
                    }
                    if (isset($key2['#below'])) {
                        foreach($key2['#below'] as &$key3):

                            if (isset($key3['#theme'])) {
                                $key3['#theme'] = 'menu_link__menu_mobile';
                            }
                            if (isset($key3['#below'])) {
                                if (isset($key3['#below']['#theme_wrappers'])) {
                                    $key3['#below']['#theme_wrappers'] = array('menu_tree__menu_mobile_below');
                                }
                                foreach($key3['#below'] as &$key4):
                                    if (isset($key4['#theme'])) {
                                        $key4['#theme'] = 'menu_link__menu_mobile';
                                    }
...
function porto_item_list($variables) {
  ...
  return '<div class="p-pagination">'.$output.'</div>';
